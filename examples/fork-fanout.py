"""Explore once, fork many: one agent reads the change into its context, one
native fork per review dimension works from that cached context, one fork of
the explorer per dimension verifies that dimension's findings, and one agent
writes the report. The change is read once at full price. The reviewers and,
by default, the verifiers start from the explorer's cached prefix; the
synthesizer (and any verify_route verifiers) are fresh. See
references/fork-fanout.md.
...
META = {"name": "fork-fanout", "description": "explore once, fork one reviewer and one verifier per dimension"}
